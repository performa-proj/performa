...
export default function EditPriceStructureDialog({
  ...
  index: number,
  predata: IPriceStructure,
  open: boolean;
  onDeleted: (index: number) => void;
  onUpdated: (structure: IPriceStructure, index: number) => void;
  onClose: () => void;
}) {
  ...
  const handleStructureDeleted = async () => {
    const payloads = {
      _id: predata._id,
    };

    await PriceStructures.deletePriceStructure(payloads);
    onDeleted(index);
  };
  ...
  const handleStructureUpdated = async () => {
    ...
    const json = await PriceStructures.updatePriceStructure(payloads);
    const data = {
      ...predata,
      title: json.title,
      levels: json.levels,
  ...
  return (
    <DialogBase
      ...
      deleteButton={{
        title: "Delete",
        onDelete: handleStructureDeleted,
      }}
      submitButton={{
        title: "Save",
        onSubmit: handleStructureUpdated,
      }}
    >
